...
Rectangle {
    ...
    Rectangle{
        ...
        CustomButton{
            ...
            onClicked: {
                if (bookReturn.hasPendingFine()) {
                    // Show fine payment dialog
                    fineDialog.fineAmount = bookReturn.getPendingFineAmount()
                    fineDialog.bookReturnHandler = bookReturn
                    fineDialog.open()
                } else {
                    // No fine, proceed with return directly
                    if (bookReturn.returnBookWithFine(true)) {
                        successDialog.open()
                    }
                }
            }
    ...
    Dialog {
        ...
        property double fineAmount: bookReturn.fineAmount
        property var bookReturnHandler: null
        ...
        ColumnLayout {
            ...
            Label {
                text: "Outstanding Fine Amount:"
                font.bold: true
            }

            Label {
                text: "KSh " + fineDialog.fineAmount.toFixed(2)
                font.pixelSize: 24
                color: "red"
            }
            ...
            Label {
                text: "Has the patron paid this fine?"
                font.bold: true
            }

            Label {
                text: "Clicking 'OK' confirms that payment has been received."
                font.italic: true
                font.pixelSize: 12
        ...
        onAccepted: {
            // Record the fine payment in issued_books table
            if (bookReturnHandler.recordFinePayment(fineAmount)) {
                // Then return the book
                if (bookReturnHandler.returnBookWithFine(true)) {
                    successDialog.open()
                }
            }
        }
